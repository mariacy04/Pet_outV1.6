...
private static final int FIELD_TYPE_FLOAT=2, FIELD_TYPE_BLOD=4, FIELD_TYPE_STRING=3, FIELD_TYPE_INT=1, FIELD_TYPE_NULL=0;
    private static final String NAME_DB = "petOutDb.db";
    private static final int VERSION_DB = 1;
    private static final String TAG_DB = "_DataBaseHelper_";
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL(MessageFormat.format("CREATE TABLE {0}({1})", StepDAO.NAME, StepDAO.getColumnsSQL()));
    }

    @Override
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        db.execSQL(MessageFormat.format("DROP TABLE {0}",StepDAO.NAME));
        onCreate(db);
    }
